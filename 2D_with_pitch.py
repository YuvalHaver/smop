...
def one_step(u_x_prev, u_z_prev, x_arr, z_arr, theta, theta_1, theta_2, phi, phi_1, phi_2, gamma, gamma_1, gamma_2):
    ...
    theta_2.append((pitch + I_z * phi_1[-1] * np.cos(theta[-1])*(phi_1[-1] * np.sin(theta[-1])+ gamma_1[-1])-I_z * phi_1[-1]**2 * np.cos(theta) * np.sin(theta))/ I_x)
    ...
    phi_2.append(roll + I_x * theta_1[-1] *phi_1[-1 ]* np.sin(theta[-1]) - I_z * theta_1[-1] *( phi_1[-1] * np.sin(theta[-1]) + gamma_1[-1]) + I_x * theta_1[-1] * phi_1 [-1])* np.cos(theta[-1]) / I_x
    ...
    gamma_2.append( (spin_down - I_z * phi_1[-1] * np.sin( theta) + theta_1[-1] * phi_1[-1] * np.cos(theta[-1]))/I_z)
